...
function EMap()
{
    ...
    */
    ...
    this.on = (key, method) =>
    {
        ...
        {
            const methods = this.events.get(key)
            this.events.set(key, [...methods, method])
        }
        else
            this.events.set(key, [method])
    }
    this.off = (key, method) =>
    {
    ...
    this.set = (key, value, method) =>
    {
        ...
        if (method)
        {
            const keyEvents = this.events.get(key)
            if (!keyEvents)
                this.events.set(key, [method])
            else
                this.events.set(key, [...keyEvents, method])
        }

        if (this.events.has(key))
            this.events.get(key).forEach((m)=>
            {
                const event = {type: 'set', key: key, loaded: value.loaded, total: value.total}
                m(event)
                console.log("fondle me with transfers ui", event)
            })
    }
    this.del = (key) =>
    {
